...
export const NoteRepository = {
  ...
  async save(note: Partial<NoteRecord> & { id?: string }): Promise<NoteRecord> {
    ...
    const record: NoteRecord = {
      ...
      podcast_script: note.podcast_script,
  ...
  /**
   * Update audio file reference for a note
   */
  async setAudioFileId(noteId: string, fileId: string): Promise<void> {
    const note = await this.get(noteId);
    if (note) {
      await this.save({ ...note, audio_file_id: fileId });
    }
  },
